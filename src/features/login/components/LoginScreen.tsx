...
import { readGoogleSignInDiagnostic } from '@/services/auth/GoogleSignInDiagnostics';
...
export function LoginScreen({ onAuthenticated }: LoginScreenProps) {
  ...
  const handleGooglePress = useCallback(async () => {
    ...
    try {
      ...
      const diagnostic = readGoogleSignInDiagnostic(error);
      const details = [
        `Etapa: ${diagnostic.stage}`,
        diagnostic.code ? `error.code: ${diagnostic.code}` : undefined,
        `error.message: ${diagnostic.message}`,
      ].filter((detail): detail is string => Boolean(detail));

      Alert.alert('Diagnóstico temporário — Login Google', details.join('\n\n'));
    } finally {
      if (isMounted.current) {
        setIsLoading(false);
      }
