...
export type GoogleSignInDiagnosticStage =
  | 'GoogleSignin.configure'
  | 'GoogleSignin.signIn'
  | 'obtenção do idToken'
  | 'GoogleAuthProvider.credential'
  | 'Firebase signInWithCredential'
  | 'validação do e-mail autorizado';

function readDiagnosticStage(causeValue: unknown): GoogleSignInDiagnosticStage | undefined {
  if (typeof causeValue !== 'object' || causeValue === null) return undefined;

  const stage = (causeValue as { diagnosticStage?: unknown }).diagnosticStage;
  return typeof stage === 'string' ? (stage as GoogleSignInDiagnosticStage) : undefined;
}
...
export class AuthUserFacingError extends Error {
  ...
  public constructor(
    ...
    message: string,
    causeValue?: unknown,
    diagnosticStage?: GoogleSignInDiagnosticStage,
  ) {
    super(message);
    this.name = 'AuthUserFacingError';
    this.code = code;
    this.causeValue = causeValue;
    this.diagnosticStage = diagnosticStage ?? readDiagnosticStage(causeValue);
  }
}
